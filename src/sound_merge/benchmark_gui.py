    ...
    try:
        ...
        if (
            ...
            messagebox.showerror(
                "Error",
                "Please ensure all paths exist and the number of files is positive.",
            )
            return

        messagebox.showinfo(
            "Success", "Benchmark started. Check the console for progress."
        )
...
def simple_layout(benchmark):
    ...
    tk.Button(root, text="Browse", command=lambda: select_directory(dir1_entry)).grid(
        row=1, column=2
    )
    ...
    tk.Button(root, text="Browse", command=lambda: select_directory(dir2_entry)).grid(
        row=2, column=2
    )
    ...
    tk.Button(root, text="Browse", command=lambda: select_directory(dest_entry)).grid(
        row=3, column=2
    )
    ...
    tk.Button(
        ...
        command=lambda: start_benchmark(
            num_files_entry,
            dir1_entry,
            dir2_entry,
            dest_entry,
...
def dynamic_select_layout(benchmark):
    ...
    def add_directory():
        ...
        tk.Label(root, text=f"Source Directory {len(directories)+1}:").grid(
            row=row, column=0
        )
        dir_entry = tk.Entry(root)
        dir_entry.grid(row=row, column=1)
        tk.Button(
            root, text="Browse", command=lambda: select_directory(dir_entry)
    ...
    tk.Button(root, text="Start Benchmark", command=start_dynamic_benchmark).grid(
        row=len(directories) + 20, column=0, columnspan=3
    )
