...
class GenAudioFile:
    ...
    - length_ms (int)
    """
    def __init__(self, mix_func: Callable, norm_func: Callable, augm_funcs: list[Callable], final_dbfs: float):
    ...
    def __call__(self, audio_files: list[Path], **kwargs: Any) -> AudioSegment:
        ...
        for segment in processed_segments:
            mixed_segment = self._mix_func(sc1=1, sc2=0.5, audio_segment1=mixed_segment, audio_segment2=segment)
            mixed_segment = self._norm_func(segment=mixed_segment, target_dBFS=self._final_dbfs)
