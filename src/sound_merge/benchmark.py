...
def simple_benchmark(num, dest, dir1, dir2, percentile1, percentile2, distr, progress_bar=None):
...
def dynamic_select_benchmark(num, dist, dirs, p, progress_bar):
    dist, dir1, dir2 = Path(dist), Path(dir1), Path(dir2)
    ...
    for i in range(num):
        ...
        logging.info(f"Finanl volume: {chosen_volume}")
        mixed_segment = mixed_segment.apply_gain(chosen_volume - mixed_segment.dBFS)
        mixed_segment.export(dist / f"audio{i+1}.wav", format='wav')
